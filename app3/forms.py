...
kalender_wochen         = [("ganzes Jahr", "ganzes Jahr")] + [mkTuple(f"KW {a}") for a in range(52)]
betriebsstrategien      = map(mkTuple, ["Optimierung nach Preis", "Optimierung nach Emissionen", "TODO"])
...
jahreszeiten            = map(mkTuple, ["Frühling", "Sommer", "Herbst", "Winter"])
...
class Scenario(Form):
    ...
    use_case                     = ChoiceField(initial='Bivalen', choices = use_cases)
    #auswahl_parametrisierung     = SubmitField(label='Parametrisierung...')
    regelleistungscheck          = BooleanField(initial=True, required=False)
    zeitfenster                  = DecimalField(initial=10)
    netzentgelte                 = DecimalField(initial=7.3)
    eeg_umlage                   = DecimalField(initial=6.8)
    stromsteuer                  = DecimalField(initial=2.1)  # DecimalField(initial=2.1)
    ...
    bhkw_kosten_pro_schaltvorgang           = DecimalField(initial=100)
    ...
    bhkw_maximaler_ladezustand              = DecimalField(initial=300)
    bhkw_standby_verluste_waermespeicher    = DecimalField(initial=0)
    bhkw_waermeverlust_relativ_ladezustand  = DecimalField(initial=99.83)
    ...
    bhkw_konstanter_waermebedarf            = DecimalField(initial=0.5)
...
use_cases = ["Direkte_Wärme-_oder_Kälteversorgung_eines_Produktionsprozesses",
            "Wärme-_oder_Kälteversorgung_eines_Produktionsprozesses_mit_vorhandenem_thermischen_Speicher",
                "Druckluftversorgung_eines_Produktionsprozesses",
                "..."]
